...
GLuint ShaderManager::LoadShaders(const char* vertex_file_path, const char* fragment_file_path)
{
    ...
    // Ensure attribute locations match the mesh layout
    glBindAttribLocation(m_programID, 0, "inVertexPosition");
    glBindAttribLocation(m_programID, 1, "inVertexNormal");
    glBindAttribLocation(m_programID, 2, "inTextureCoordinate");
